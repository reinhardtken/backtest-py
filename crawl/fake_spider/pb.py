  ...
  def __next__(self):
    if self.counter < len(self.codes):
      ...
      if code[0] == '0' or code[0] == '3':
        head = 'sz'
      url = "https://eniu.com/chart/pba/{0}{1}".format(
        head, code)
...
class Handler(spider2.FakeSpider):
  ...
  def url(self):
    url = "https://eniu.com/chart/pba/sh{0}".format("601398")
    out = []
    out.append(url)
    return out
  ...
  def processFirstPage(self, response):
    ...
    try:
      ...
      jsonData["_id"] = urls[-1]
      util.saveMongoDB2(jsonData, "stock2", "pb")
...
def transfer(code):
  ...
  if len(date) == len(allPrice) and len(date) == len(dv):
    ...
    for index in range(len(date)):
      ...
      out["data"].append(tmp)
    util.saveMongoDB2(out, "stock2", "pb2")
  else:
