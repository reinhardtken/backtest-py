...
class MyNumbers:
  ...
  def __next__(self):
    return ("https://eniu.com/chart/peindex/sh000016", self.kwargs)
    return ("https://eniu.com/chart/peindex/sz399300", self.kwargs)
    if self.counter < len(self.codes):
      code = self.codes[self.counter]
      head = "sh"
      if code[0] == '0' or code[0] == '3':
        head = 'sz'
      url = "https://eniu.com/chart/pea/{0}{1}".format(
        head, code)
...
class Handler(spider2.FakeSpider):
  ...
  def url(self):
    url = "https://eniu.com/chart/pea/sh{0}".format("601398")
    out = []
    out.append(url)
    return out
  ...
  def processFirstPage(self, response):
    ...
    try:
      ...
      jsonData["_id"] = urls[-1]
      util.saveMongoDB2(jsonData, "stock2", "pe")
